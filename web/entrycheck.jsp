...
if( user==null || pswd==null)
{
	...
	response.sendRedirect("./pc.jsp?err=1&"+paraString);
}
else
{
	user = user.trim();
	...
	if(user.length() == 0 || pswd.length() == 0)
	{
		...
		//	out.print("用户名和密码不能为空，请修改后重试。 <br/>");	
		response.sendRedirect("./pc.jsp?_user="+user+"&_pswd="+pswd+"&err=1&"+paraString);
	}
	else if( user.length()<2 || pswd.length()<2 )
	{
		//out.print("|"+user + "|:|" + pswd + "|<br/>");
		//out.print("为了你的安全，用户名和密码不能少于2个字符，请修改后重试。 <br/>");	
		response.sendRedirect("./pc.jsp?_user="+user+"&_pswd="+pswd+"&err=2&"+paraString);
	}
	else if( user.length()>12 || pswd.length()>12 )
	{
		//resultData += "游戏账号和密码必须是2~12位的英文或者数字，或者两者的组合<br/>";
		response.sendRedirect("./pc.jsp?_user="+user+"&_pswd="+pswd+"&err=5&"+paraString);
	}
	else
	{	
		...
		if(!isLegalChar(user_pswd))
		{
			...
			response.sendRedirect("./pc.jsp?_user="+user_url+"&_pswd="+pswd_url+"&err=3&"+paraString);
		}
		else 
		{
			...
			if(ret.equals("error1"))
			{
				...
				response.sendRedirect("./pc.jsp?_user="+user+"&_pswd="+pswd+"&err=4&"+paraString);
			}
			else if(ret.equals("error2"))
			{
				//title += "您输入的用户名不存在，是否要注册这个帐户?\n";
				response.sendRedirect("./regnew.jsp?_user="+user+"&_pswd="+pswd+"&_game_pre="+game_pre+"&err=6&"+paraString);
			}
			else if(ret.equals("error3"))
			{
				//title += "您输入的用户名和密码认证失败，是否需要找回密码？\n";
				response.sendRedirect("./pc.jsp?_user="+user+"&_pswd="+pswd+"&err=6&"+paraString);
			}
			else if(ret.equals("error4"))
			{
				 //严重错误，前台没有严格控制传入合法的用户名密码  
				response.sendRedirect("./pc.jsp?_user="+user+"&_pswd="+pswd+"&err=7&"+paraString);
			}
			else
			{
			   response.sendRedirect("./do.jsp?_user="+user_new+"&_pswd="+pswd+"&regnewFlag="+regnewFlag);   
			}
		}
	}
}
