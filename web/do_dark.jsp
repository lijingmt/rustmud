...
<%!
String jspname="do_dark.jsp";
String filter_type="html20240101_dark";
...
if(first_login){	
	...
	//if(_ip.equals("116.26.97.151"))
	//	return;
	
	if(diffTime<=300){
		isession.setAttribute("ot",time_nt);
		//String testua = (String)request.getHeader("User-Agent");
		//System.out.println("[tx/do_dark.jsp login uid=]["+uid+"] IP=["+_ip+"] UA = ["+testua+"] click time difftime<=["+String.valueOf(diffTime)+"]\n");
		return;
	}
	else
		isession.setAttribute("ot",time_nt);
	...
	if(diffTime<=800){
		...
		if(testua!=null){
			...
			if (containsString){
				...
				System.out.println("[tx/do_dark.jsp login uid=]["+uid+"] is Nokia 5800, and click time difftime=["+String.valueOf(diffTime)+"] < 800ms\n");
				//cmd = "quit";
        		//return;
			}
			else{ //���޲�𹥻��ˣ����ԣ�ֻҪ��ͨ�û���������С��400��return
				if(diffTime<=350){
					System.out.println("[tx/do_dark.jsp login uid=]["+uid+"] UA = ["+testua+"] click time difftime=["+String.valueOf(diffTime)+"] < 350ms\n");
					uaid="Nokia";
					//cmd = "quit";
					//return;
				}
			...
			System.out.println("[tx/do.jsp login uid=]["+uid+"] have no UA, cmd=["+cmd+"] click time difftime=["+String.valueOf(diffTime)+"] < 800ms\n");
			uaid="Nokia";
        	//cmd = "quit";
			//return;
		}
	...
	//if(uid.equals("tx0118364998111")||uid.equals("tx01q002")||uid.equals("tx01q003")||uid.equals("tx01q007"))  
	//	uaid="Nokia";
	...
	*/
	...
	String uaid="Nokia";
	//send(writer,("login "+projname+" "+uid+" "+pid+" "+usid+" "+uaid).getBytes());
	send(writer,("login "+projname+" "+uid+" "+pid+" "+usid+" "+_ip).getBytes());
}
