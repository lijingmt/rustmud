// rustenv/rustenv.rs - 主服务器入口
// 对应 txpike9/pikenv/pikenv.pike

use crate::core::{MudError, Result};
use crate::rustenv::config::CONFIG;
use crate::rustenv::conn::accept_connection;
use crate::rustenv::gc_manager::GcManager;
...
/// Rustenv 主服务器
pub struct RustenvServer {
    config: &'static crate::rustenv::config::Config,
}
...
impl Default for RustenvServer {
    fn default() -> Self {
        Self::new()
    }
}
...
async fn main() -> Result<()> {
    ...
    let mut server = RustenvServer::new();
...
fn print_usage() {
    ...
    println!();
    println!("Usage: rustenv [OPTIONS] <mudlib directory> [script.pike] ...");
    println!();
    println!("Options:");
